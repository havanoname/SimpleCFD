# -*- coding: utf-8 -*-
# @Time    :   2021/12/02 00:45:56
# @FileName:   implicit_scheme_UDS.py
# @Author  :   ljy
# @Software:   VSCode
# 隐式格式-一阶迎风
...
def matReshape(Ta, Tb, a, a0, dx, S, V, count, t):  # 重组矩阵成为系数矩阵
    ...
    b = b + a0 * np.reshape(numerical_T[:, t-1], (count, 1))
...
def fun():
    ...
    for t in range(1, len(time)):
        ...
        a_temp = a
        a_temp[:, 0:2, t] = -1*a_temp[:, 0:2, t]  # 对aw和ae取负表示移项
        # 交换ap和ae，使得ap在中间，因为matReshape函数假设ap在中间
        a_temp[:, [1, 2], t] = a_temp[:, [2, 1], t]

        A, b = matReshape(Ta, Tb, a_temp[:, :, t], a0, dx, S, V, count, t)
