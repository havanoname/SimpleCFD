...
def PLS(Gamma, dx, rho, u, Sc, Sp, A, V, L, i):  # 每一段控制体进行计算
    D = Gamma / dx
    F = rho * u
    ...
    a[i, 0] = D[0]*max(0, (1-0.1*abs(Pe[0]))**5) + max(F[0], 0)
    a[i, 1] = D[1]*max(0, (1-0.1*abs(Pe[1]))**5) + max(-F[1], 0)
...
def fun():
    ...
    Sc, Sp = 0, 0  # 无源场
    u = np.array([0.2, 0.2], dtype=np.float64)  # 风速 u_w and u_e
    ...
    for i in range(0, count):
        ...
        PLS(Gamma, dx[i], rho, u, Sc, Sp, A, V, L, i)
