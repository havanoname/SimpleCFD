...
def EDS(Gamma, dx, rho, u, Sc, Sp, A, V, L, i):  # 每一段控制体进行计算
    D = Gamma / dx
    F = rho * u
    ...
    a[i, 0] = F[0] * math.exp(F[0]/D[0]) / (math.exp(F[0]/D[0])-1)
    a[i, 1] = F[1] / (math.exp(F[1]/D[1])-1)
...
def fun():
    ...
    for i in range(0, count):
        ...
        EDS(Gamma, dx[i], rho, u, Sc, Sp, A, V, L, i)
