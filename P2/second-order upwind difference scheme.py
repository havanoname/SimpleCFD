...
def UDS(Gamma, dx, rho, u, A, V, L, i):  # 每一段控制体进行计算
    D = Gamma / dx
    F = rho * u
    ...
    a[i, 0] = -1/2 * alpha * F[0]  # aww
    a[i, 1] = D[0] + 3/2 * alpha * F[0] + 1/2 * alpha * F[1]  # aw
    a[i, 2] = D[1] - 3/2 * (1-alpha) * F[1] - 1 / \
        2 * (1-alpha) * F[0]   # ae
    a[i, 3] = 1/2 * (1-alpha) * F[1]  # aee
