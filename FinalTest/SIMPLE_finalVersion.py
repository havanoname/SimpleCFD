...
rho = 1  # 密度为kg/m3
gamma = 0  # 无摩擦, 黏性扩散项=0
relax_factors_u = 0.01  # 速度松弛因子
relax_factors_p = 0.01  # 压力松弛因子


# 网格信息
nodes_P = 50  # 压力节点数 A B C D E
...
x_P = np.expand_dims(x_P, axis=1)  # 升维,保证数组统一
...
p_calculated = np.zeros((nodes_P, 1))  # 记为p*
p_new = np.zeros((nodes_P, 1))

iteration = 0
mome_residual = 1  # 用于激活循环
res_record = [mome_residual]

while (mome_residual > 10**-6 or iteration > 10**5):
    if iteration == 0:
        u_old = u_init
        p_old = p_init
        # alpha_relax = 1
    ...
    for i in range(0, nodes_V):
        if i != 0 and i != nodes_V-1:  # 计算中间节点 如2 3
            F_v[i][0] = rho * ((u_old[i-1] + u_old[i])/2) * A_xP[i][0]  # Fw
            F_v[i][1] = rho * ((u_old[i] + u_old[i+1])/2) * A_xP[i+1][0]  # Fe
            ...
            Su_v[i] = (p_old[i][0] - p_old[i+1][0]) * \
                A_xV[i]  # 速度网格Su = delta_P * A_xV
            d_v[i] = A_xV[i] / a_v[i][2]  # d = A/ap

        # 处理边界节点 1
        if i == 0:
            uA = u_old[i][0] * A_xV[i] / A_xP[i]
            F_v[i][0] = rho * uA * A_xP[i]
            F_v[i][1] = rho * (u_old[i][0] + u_old[i+1][0])/2 * A_xP[i+1][0]
            D_v[i][0] = 0  # Dw
            ...
            # Su_v[i] = (p_old[i][0] - p_old[i+1][0])*A_xV[i] + \
            #    F_v[i][0] * (A_xV[i]/A_xP[i])*u_old[i][0]  # 压力p0-pB
            Su_v[i] = (P_in - p_old[i+1][0])*A_xV[i] + \
                F_v[i][0] * (A_xV[i]/A_xP[i])*u_old[i][0]  # 压力p0-pB
            d_v[i] = A_xV[i] / a_v[i][2]

        # 处理边界节点 4
        if i == nodes_V-1:
            F_v[i][0] = rho * ((u_old[i-1][0] + u_old[i][0])/2) * A_xP[i][0]
            # Fe 直接用速度网格计算..我也不知道为啥
            F_v[i][1] = rho * (u_old[i]) * A_xV[i][0]
    ...
    # 自己看P207页自己悟吧..就是把pB'和pC'移项到offest=-1
    matrix_Ap[rng+1, rng] = -1*a_p[2:nodes_V, 0]
    matrix_Ap[rng, rng+1] = -1*a_p[1:nodes_V-1, 1]  # 移另一项

    vector_bp = b_[1:nodes_P-1, 0]
    # 针对bp处理边界的A和E
    vector_bp[0] = a_p[0, 0]*p_[0, 0] + vector_bp[0]
    vector_bp[len(vector_bp)-1] = a_p[len(a_p)-1, 1]*p_[nodes_P-1,
                                                        0] + vector_bp[len(vector_bp)-1]  # 多维数组len()返回行数 即nodes_P
    vector_bp = np.expand_dims(vector_bp, axis=1)  # 升维

    p_[1:nodes_P-1] = np.linalg.solve(matrix_Ap, vector_bp)  # 左闭右开

    # 计算修正后的压力
    # p_calculated[:, 0] = (1-relax_factors)*p_old[:, 0] + relax_factors*p_[:, 0]
    p_calculated[1:nodes_P-1] = p_old[1:nodes_P-1] + \
        p_[1:nodes_P-1]  # 计算中间点的修正压力
    # 计算修正速度
    delta_p_ = np.zeros((nodes_V, 1))
    for i in range(0, nodes_V):
        delta_p_[i, 0] = p_[i, 0]-p_[i+1, 0]  # 修正压力差！注意是修正压力！！！
    u_calculated = u_ + d_v*delta_p_

    # 计算入口A点的修正后的压力
    p_calculated[0, 0] = P_in - 1/2 * rho * \
        u_calculated[0, 0]**2 * (A_xV[0, 0]/A_xP[0, 0])**2
    # 连续性检查
    conti_check = rho * u_calculated * A_xV  # 所有元素一样则是正确的

    # 引入松弛因子
    u_new = (1 - relax_factors_u) * u_old + relax_factors_u * u_calculated
    # p_new[1:nodes_P-1] = (1-relax_factors) * p_old[1:nodes_P-1] + \
    #     relax_factors*p_calculated[1:nodes_P-1]
    p_new = (1 - relax_factors_p) * p_old + relax_factors_p * p_calculated

    # 收敛误差, 参考An Introduction to Computational Fluid Dynamics_ The Finite Volume Method P209
    # # error = abs(max(u_new[:, 0] - u_old[:, 0]) / max(u_new[:, 0]))
    # 速度残差 以节点1为参考, 因为节点1好算
    mome_residual = abs(Su_v[0, 0] - a_v[0, 2]*u_old[0, 0])
    # mome_residual = sum(abs(Su_v[:,0]-a_v[:,2]*u_old[:,0]))/nodes_V #  取残差和求平均,如果用向量范数则迭代更久
    res_record = np.append(res_record, mome_residual)
    # 进入下一步迭代
    if iteration == 19:  # 偶尔检查一下结果
        print(p_new)
        print(u_new)
...
# 解析解
m_f = 0.44721
exact_pN = P_in - 1/2 * rho * m_f**2 / (rho * A_xP)**2

exact_pNForuN = P_in - 1/2 * rho * m_f**2 / (rho * A_xV)**2
exact_uN = np.sqrt((P_in - exact_pNForuN)*2 / rho)

plt.figure()
plt.grid(b=True)
plt.plot(range(1, nodes_P+1), p_new, 'r*-', label='numerical solution')
plt.plot(range(1, nodes_P+1), exact_pN, 'bo-', label='exact solution')
plt.xlabel(
    "Nodes of pressure, under-relaxation for pressure: {}".format(relax_factors_p))
plt.ylabel("Pressure")
plt.legend()
# plt.title('iterations: {}, momentum residual: {}'.format(iteration, mome_residual))
plt.title('iterations: {}, nodes of pressure: {}'.format(iteration, nodes_P))

plt.figure()
plt.grid(b=True)
plt.plot(range(1, nodes_V+1), u_new, 'g*-', label='numerical solution')
plt.plot(range(1, nodes_V+1), exact_uN, 'bo-', label='exact solution')
plt.xlabel(
    "Nodes of velocity, under-relaxation for velocity: {}".format(relax_factors_u))
plt.ylabel("Velocity")
plt.legend()
plt.title('momentum residual: {}, nodes of velocity: {}'.format(
    mome_residual, nodes_V))

plt.figure()
plt.plot(range(1, iteration), res_record[1:iteration])
plt.xlabel("Iterations")
plt.ylabel("momentum residual")
plt.title("momentum residual: {}".format(mome_residual))
