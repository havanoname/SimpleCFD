...
while (error > 10**-6):
    ...
    for i in range(0, nodes_V):
        ...
        if i == nodes_V-1:
            ...
            Su_v[i] = (p_old[i-1][0] - p_old[i][0])*A_xV[i]
            d_v[i] = A_xV[i] / a_v[i][2]
    ...
    # 计算修正压力
    # p_calculated[1:nodes_P-1] = p_old[1:nodes_P-1] + p_[1:nodes_P-1]
    # 计算修正速度
    ...
    # 更新新的速度, 用新速度算压力
    u_new = (1-relax_factors) * u_old + relax_factors*u_calculated
    
    # 计算入口A点的修正压力
    p_calculated[0,0] = P_in - 1/2 * rho * u_new[0,0]**2 * (A_xV[0,0]/A_xP[0,0])**2
    
    delta_u = np.zeros((nodes_V-1,1))
    for i in range(1, nodes_V-1):
        delta_u[i,0] = u_new[i,0] - u_new[i-1,0]
    p_calculated[1:nodes_P-1] = (p_[1:nodes_P-1] - d_p[1:nodes_P-1] * delta_u)
    ...
    p_new = (1-relax_factors) * p_old + relax_factors*p_calculated
